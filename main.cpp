    ...
    class vector { 
        ...
        public:
            ...
            vector(std::initializer_list<T> set) 
                ...
                std::uninitialized_move(set.begin(), set.end(), reinterpret_cast<T*>(buffer_));
            }

            vector(std::size_t n, const T& default_value = T()) 
                    : size_(n), capacity_(n), buffer_(new char[sizeof(T) * n]) {
                std::uninitialized_fill_n(reinterpret_cast<T*>(buffer_), n, default_value);
            }

            vector(const vector<T>& temp) 
                    : size_(temp.size()), capacity_(temp.size()),
                      buffer_(new char[sizeof(T) * temp.size()]) {
                std::uninitialized_copy(temp.begin(), temp.end(), reinterpret_cast<T*>(buffer_));
            }
            
            vector(vector<T>&& temp) 
                    : size_(temp.size()), capacity_(temp.size()), buffer_(temp.buffer_) {
                temp.size_ = 0;
                temp.capacity_ = 0;
            ...
            vector& operator =(const vector<T>& other) { 
                ...
                std::uninitialized_copy(other.begin(), other.end(), reinterpret_cast<T*>(buffer_));
                return *this;
            }
            ...
            void push_back(T&& argument) {
                if (size_ == capacity_) { 
                    char* new_buffer_ = new alignas(alignof(T)) char[size_ * sizeof(T) * 2];
                ...
                new (reinterpret_cast<T*>(buffer_) + size_) T(std::move(argument));
                size_++;
            }
            ...
            void insert(vector<T>::iterator place, T&& argument) {
                ...
                if (size_ == capacity_) { 
                    ...
                    new (reinterpret_cast<T*>(new_buffer_) + index) T(std::move(argument));
                    std::destroy_n(reinterpret_cast<T*>(buffer_), size_);
                    delete [] buffer_; 
                    buffer_ = new_buffer_;  
                    capacity_ = size_ * 2;
                    ...
                    *(reinterpret_cast<T*>(buffer_) + index) = std::move(argument);
                }
                size_++;
            }
            ...
            bool operator <(const vector<T>& second) const { 
                std::size_t less_size = std::min(size_, second.size_);
...
int main() { 
    ...
    my_vector::vector<int> m = {100, 100, 500, 500, 100, 100};
    v.insert(0, m.begin(), m.begin() + 3);
    v.insert(5, m.begin(), m.begin() + 3);
    for (int i : v) { 
        std::cout << i << ' ';
    }
    std::cout << '\n';
